...
已检查 TypeScript、自有源码 lint、静态生产构建，以及 14 项自动化测试（经济、指令、训练、技能、建造、波次、暂停、重开和完整胜利流程）。**尚未做 iOS / Android 真机帧率、浏览器交互、离线安装流程或无障碍辅助技术验证。** 不承诺具体设备帧率。

WebMCP 是可选的浏览器提案：浏览器不支持时照常游戏；注册失败不会阻止游戏。源码测试覆盖其参数校验和状态共用；尚未在支持该接口的浏览器中验证。
