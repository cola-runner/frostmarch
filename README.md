...
[Source](https://github.com/cola-runner/frostmarch) · [Releases](https://github.com/cola-runner/frostmarch/releases) · [Play instantly](https://frostmarch-cola.owen-garcia716.chatgpt.site)

The live game is public: open the link in your phone browser and start playing. No account or app-store download is required. The source and static build are also available for local play or independent hosting.
...
Touch Edition 0.2 adds larger thumb controls, a collapsible map, separate portrait and landscape layouts, screen-space unit selection, and independent tap/drag/pinch handling.
...
The earlier Web demo remains available as [v0.1.1](https://github.com/cola-runner/frostmarch/releases/tag/v0.1.1). This touch edition is still a browser game, not a native APK or IPA.
...
Use a modern browser with WebGL 2 and hardware acceleration. On mobile, landscape offers a wider view. Add the game to your home screen from the browser menu for a standalone PWA experience. The build precaches the complete campaign, including the game engine and UI bundles. Once the field guide shows **Campaign ready offline**, the campaign can be reopened offline. Where supported, the guide offers **Install Frostmarch**; it also includes manual home-screen steps for iPhone and Android. Browser storage may be cleared by the device. Real iOS/Android installation and device frame rates have not yet been verified.
...
The production build is a static site in **`dist/client/`**. Deploy that directory to an HTTPS static host. Routes and assets assume a domain root; subdirectory hosting requires base-path configuration. Development does not register a service worker; the static production build does, including when served from localhost.
...
TypeScript, lint for project-owned code, the static production build, and 21 automated battle/input/WebMCP checks are part of the validation workflow. Phone portrait and landscape layouts, camp planning, recruitment, and spell confirmation are checked in a browser. Tests cover resource delivery, orders, recruitment, spells, construction, waves, pause/reset behavior, and a complete winning campaign without cheats. No specific device frame rate is promised.
